...
F5R_CO2 = 0; % Initialize F5R_CO2
F5R_H2 = 0;  % Initialize F5R_H2
F5R_CH3OH = 0;  % Initialize F5R_CH3OH
F5R_H2O = 0;  % Initialize F5R_H2O
...
fprintf('The molar flow rate of stream 3 is: %d kmol/hr\n', F3_CO2 + F3_H2 + F3_CH3OH + F3_H2O);
fprintf('The molar flow rate of stream 4 is: %d kmol/hr\n', F4_CO2 + F4_H2 + F4_CH3OH + F4_H2O);
fprintf('The molar flow rate of stream 5 is: %d kmol/hr\n', F5_CO2 + F5_H2 + F5_CH3OH + F5_H2O);
fprintf('The molar flow rate of stream 5R is: %d kmol/hr\n', F5R_CO2 + F5R_H2 + F5R_CH3OH + F5R_H2O);
fprintf('The molar flow rate of stream 6 is: %d kmol/hr\n', F6_CO2 + F6_H2 + F6_CH3OH + F6_H2O);
fprintf('The molar flow rate of stream 7 is: %d kmol/hr\n', F7_CO2 + F7_H2 + F7_CH3OH + F7_H2O);
fprintf('The molar flow rate of stream 8 is: %d kmol/hr\n', F8_CO2 + F8_H2 + F8_CH3OH + F8_H2O);
fprintf('The molar flow rate of stream 9 is: %d kmol/hr\n', F9_CO2 + F9_H2 + F9_CH3OH + F9_H2O);
fprintf('The molar flow rate of stream 10 is: %d kmol/hr\n', F10_CO2 + F10_H2 + F10_CH3OH + F10_H2O);

a = F9_CH3OH;
n4 = F4_CO2 + F4_H2 + F4_CH3OH + F4_H2O;
n6 = F6_CO2 + F6_H2 + F6_CH3OH + F6_H2O;
n8 = F8_CO2 + F8_H2 + F8_CH3OH + F8_H2O;
F3 = 2000 * 44 + 4000 * 2;
F3p = F3_CO2 + F3_H2 + F3_CH3OH + F3_H2O;

pco2 = linspace(0.025, 1, 100);
ph2 = linspace(0.5, 2, 100);

C_cap = zeros(100, 100);
C_capp = zeros(100, 100);
phi = 0.1;
TAC = zeros(100, 100);
TACp = zeros(100, 100);
C_methanol = zeros(100, 100);
C_methanolp = zeros(100, 100);

for i = 1:100
    for k = 1:100
        C_cap(i, k) = 14694200 + 29418000 + (1.53 * (10^7) * ((F3 / 54000)^0.65)) + 269600 + 171300 + 168100 + 63000 + 1507900;
        C_capp(i, k) = 14694200 + 29418000 + (1.53 * (10^7) * ((F3p / 54000)^0.65)) + 269600 * (n4 / 4670)^(0.6) + 171300 * (n4 / 4670)^(0.6) + 168100 * (n6 / (1.349251e+03))^(0.6) + 63000 * (n8 / (1.302675e+03))^(0.6) + 1507900 * (n8 / (1.302675e+03))^(0.6);
        C_op(i, k) = ((9.7 + 21.5) * 72 + (0.28 + 0.8 + 15.7) * 1000 * 3600 * 2.5 * (10^-6) + (12 + 20.9 + 22.6 + 15.4) * 1000 * 3600 * 2.12 * (10^-7) + (2000 * 44) * pco2(i) + (4000 * 2) * ph2(k)) * 7884;
        TAC(i, k) = phi * C_cap(i, k) + C_op(i, k);
        TACp(i, k) = phi * C_capp(i, k) + C_op(i, k);
        C_methanol(i, k) = TAC(i, k) / (7884 * (a * 32 / 1000));
        C_methanolp(i, k) = TACp(i, k) / (7884 * (a * 32 / 1000));
    end
end
