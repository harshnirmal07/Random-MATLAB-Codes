...
% slope_r = min_reflux_ratio/(1+min_reflux_ratio);
% m = slope_op_II;
% l_bar = slope_op_III;
% 
% x_int_1 = x_side_rate;
% y_int_1 = slope_r * x_side_rate / (slope_r + 1) + xD / (slope_r + 1);
% x_int_2 = 0.3958;
% y_int_2 = l_bar * x_int_2 / (l_bar - W) - W * xW / (l_bar - W);
% 
% % Perform McCabe-Thiele construction for Section I
% X_1 = xD;
% xD = xD;
% nt = -1;
% 
...
%     X_old = xD;
%     Y_old = xD;
%     xD = y_int_1 + (xD - x_int_1) * slope_r;
%     X_1 = fsolve(@(X) xD - fit_func(fit_params, X), 0);
%     nt = nt + 1;
%     line([X_old X_old], [Y_old xD]);
%     line([X_old X_1], [xD xD]);
% end
% 
% % Perform McCabe-Thiele construction for Section II
% while (X_1 >= x_int_2)
%     X_old = X_1;
%     Y_old = xD;
%     xD = y_int_2 + (X_1 - x_int_2) * slope_2;
%     X_1 = fsolve(@(X) xD - fit_func(fit_params, X), 0);
%     nt = nt + 1;
%     line([X_old X_old], [Y_old xD]);
%     line([X_old X_1], [xD xD]);
% end
% 
% % Perform McCabe-Thiele construction for Section III
% while (X_1 >= xW)
%     X_old = X_1;
%     Y_old = xD;
%     xD = y_int_2 + (X_1 - x_int_2) * slope_3;
%     X_1 = fsolve(@(X) xD - fit_func(fit_params, X), 0);
%     nt = nt + 1;
%     line([X_old X_old], [Y_old xD]);
%     line([X_old X_1], [xD xD]);
